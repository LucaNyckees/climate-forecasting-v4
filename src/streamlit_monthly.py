...
from paths import DATAGENERATED_PATH
...
def monthly_analysis():
    ...
    data_M = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly_Mean.csv")

    st.markdown(
        r"In the previous analysis, we briefly questioned the relevance of considering the average annual temperatures in Geneva, given that during a year the range of observed temperatures is quite large. However, we are interested here in modelling the average monthly temperatures observed in Geneva Observatory, which makes these concerns less relevant since the temperatures in any one month are all much more similar."
    )
    st.markdown(
        r"In this analysis, we will try to model the time series of monthly average temperature at the Geneva Observatory. Through this model we will try to see if we can detect a significant increase in the mean temperature trend as we have been able to do with the annual means. To begin, we can see on the following figure the interactive plot of the entire time serie of the monthly mean temperature. The interactive part of the plot is the window on which the time series is plotted, we can set the window to a given size and scroll it."
    )
    ...
    for i in range(1, 13):
        fig.add_trace(
            go.Scatter(
                x=data_M[data_M.Month == float(i)].Years,
    ...
    st.markdown(
        "On the plots above a slight increase in the trend can be seen, but it is much less pronounced than in the time series of annual averages. Similarly, in the figure below we plot the standard temperature anomalies by month, but again it is more difficult to detect an increase in the trend."
    )
    ...
    fig.update_layout(
        updatemenus=[
            dict(
                ...
                buttons=list(
                    [
                        dict(label="Seasonnal estimation", method="update", args=[{"visible": [False, True]}]),
                        dict(
                            label="Monthly mean and seasonnal estimate",
                            method="update",
    ...
    fig["layout"].update(
        {
            ...
            "title_x": 0.5,
            "xaxis": {"zeroline": False},
            "yaxis": {"title": "Residus of the seasonal estimation (°C)"},
            "width": 1200,
            "height": 500,
            "legend": {"title": ""},
        }
    ...
    pval_ind1 = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly1_pvalue_indep_GLS")
    ...
    fig.add_trace(
        go.Scatter(x=pval_ind1.index + 1, y=pval_ind1.McLeod, mode="lines+markers", name="McLeod-Li"), row=2, col=2
    )
    ...
    for i in range(5):
        fig.add_trace(
            go.Scatter(
                x=p,
                y=aic1[aic1.param_q == i].AIC,
                mode="lines+markers",
                name="q = " + str(i),
                marker=dict(size=12, color=px.colors.qualitative.Dark24[i]),
            ),
            row=1,
            col=2,
        )
    ...
    resid_arma4_3 = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly_arma4_3.csv")
    ...
    pval_ind2 = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly2_pvalue_indep_GLS")
    ...
    fig.add_trace(
        go.Scatter(x=np.arange(8, 50), y=pval_ind2.Ljung[8:], mode="lines+markers", name="Ljung-Box"), row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=np.arange(8, 50), y=pval_ind2.McLeod[8:], mode="lines+markers", name="McLeod-Li"), row=1, col=2
    )
    fig.add_hline(y=0.05, line_dash="dash", line_color="red", name="alpha = 0.05")
    ...
    monthly2_seasonless_gls = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly2_seasonless_GLS.csv")
    ...
    fig.add_trace(
        go.Scatter(
            x=monthly2_seasonless_gls.Date,
    ...
    for i in range(1, 13):
        fig.add_trace(
            go.Scatter(
                x=monthly2_seasonless_gls[monthly2_seasonless_gls.Month == float(i)].Years,
    ...
    col1, col2 = st.columns([1, 1])
    with col1:
        HtmlFile = open(DATAGENERATED_PATH / "Monthly/summary2_GLS_personaliser", "r", encoding="utf-8")
        source_code = HtmlFile.read()
        st.markdown(source_code, unsafe_allow_html=True)

    with col2:
        HtmlFile = open(DATAGENERATED_PATH / "Monthly/summary2_GLS_personaliser1", "r", encoding="utf-8")
        source_code = HtmlFile.read()
        st.markdown(source_code, unsafe_allow_html=True)

    st.markdown(
        r"In the table on the right above, the GLS regression estimated all $\beta_i$ parameters as positive and with only two parameters, $\beta_5$ and $\beta_9$ (i.e. $x17$ and $x21$), that do not have a 95% confidence interval strictly greater than zero.Thus, if our model subsequently proves to be appropriate, these confidence intervals for $\beta_{i} , i \in \{1, ..., 12\}\setminus \{5,9\}$ would be an argument for the presence of a significant increase in the trend of monthly mean temperatures, except for the month of May and September."
    )
    ...
    pval_ind4 = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly4_pvalue_indep_GLS")
    ...
    fig.add_trace(
        go.Scatter(x=pval_ind4.index + 1, y=pval_ind4.Ljung, mode="lines+markers", name="Ljung-Box"), row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=pval_ind4.index + 1, y=pval_ind4.McLeod, mode="lines+markers", name="McLeod-Li"), row=2, col=2
    )
    fig.add_hline(y=0.05, line_dash="dash", line_color="red", name="alpha = 0.05")
    ...
    resid_arma5_4 = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly_resid_ARMA5_4.csv")
    ...
    pval_ind4_2 = pd.read_csv(DATAGENERATED_PATH / "Monthly/Monthly4_pvalue_indep_arma5_4")
    ...
    fig.add_trace(
        go.Scatter(x=np.arange(10, 50), y=pval_ind4_2.Ljung[10:], mode="lines+markers", name="Ljung-Box"), row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=np.arange(10, 50), y=pval_ind4_2.McLeod[10:], mode="lines+markers", name="McLeod-Li"), row=1, col=2
    )
    fig.add_hline(y=0.05, line_dash="dash", line_color="red", name="alpha = 0.05")
    ...
    st.markdown(
        r"As the p-value of the Augmented Dickey-Fuller test is $p_{value} = 0$, we can reject the null hypothesis of non-stationarity, and thus compute the acf and pacf that we could see on the plot below. These two plots confirms that the residuals appear to be independent."
    )

    st.image("figure/Monthly/resid_arma_4_acf_pacf.png")

    st.markdown(
        r"Nevertheless, after performing the Jarque-Bera test and the Goldfeld-Quandt test, we get the p-values : "
    )
