...
from plotly_features import (
    ...
    plotly_max,
    plotly_std,
    plotly_mean_temp_global,
    circular_vision,
    all_obs_precise,
    all_obs,
)
from helpers import data_processing, geo_correlation_net
...
def st_geo_correlation_net(dfs, names):
    ...
    st.markdown(
        "For each slider value, we plot a threshold-network induced by the correlation matrix between each station."
    )
    ...
    with st.expander("See explanation"):
        st.markdown(
            "For $k_1,k_2\in\{1901,1902,...,2020\}$ and an observatory $i$, let $\mathbf{x}_{k_1,k_2}^i$ be the vector of mean temperatures recorded from year $k_1$ to year $k_2$ at observatory $i$. We define the *sample Pearson correlation coefficient between two vectors* $\mathbf{x}$ and $\mathbf{y}$ of size $n$ as the quantity"
        )
...
def st_all_obs_curves(dfs, dfs_M, names):
    ...
    st.markdown(
        "This feature enables a time-window visualization of mean temperature over all observatories in Switzerland."
    )
...
def st_circular_vision():
    data_temperature = data_processing(DATA_PATH / "observatoire-geneve" / "TG_STAID000241.txt")
...
def st_display_pdf(pdf_file):
    ...
    pdf_display = (
        f'<embed src="data:application/pdf;base64,{base64_pdf}" width="1000" height="1000" type="application/pdf">'
    )
...
def correlation_net(df, elt):
    ...
    fig.update_layout(title="Correlation matrix - a vector is one year's mean temperatures")

    l.plotly_chart(fig)

    with st.expander("See explanation"):
        st.markdown(
            "For $k\in\{1901,1902,...,2020\}$, let $\mathbf{x}_k$ be the vector of mean temperatures recorded in year $k$. we define the *sample Pearson correlation coefficient between two vectors* $x$ and $y$ of size $n$ as the quantity"
        )
        ...
        """)
        st.markdown(
            "For two integer indices $i,j\in\{1901,1902,...,2020\}$, we define a matrix entry $M_{i,j}$ as $\mathrm{Corr}(\mathbf{x}_i,\mathbf{x}_j)$. Now, depending on the choice of time-window $\Omega=\{m_{start},m_{start}+1,...,m_{end}\}$ (*i.e.* a finite range of consecutive years), we plot the correlation matrix $M=(M_{i,j})_{i,j\in\Omega}$ as a regular heatmap (*cf.* the colorbar on the side)."
        )

    st.markdown(
        "For each slider value, we plot a threshold-network induced by the correlation matrix between each year."
    )
...
def multiple_curves_window(df, elt):
    ...
    st.markdown(
        "For visual simplicity and better interpretation, we recommend to choose a small time-window (e.g. 2 to 4 years)."
    )
...
def description():
    ...
    st.markdown(
        "Hey there! This app is developped in the context of a project supervised by Prof. [Mehdi Gholam](https://people.epfl.ch/mehdi.gholam?lang=fr) at EPFL, in the context of the course *Statistical Computation and Visualization*."
    )
    ...
    st.write(
        "_Is there a significant increase in the average temperature trend in Switzerland from 1901 to the present day ?_"
    )
...
def datasets():
    st.sidebar.markdown(
        "The data we work with can be found [here](https://www.ecad.eu/utils/showselection.php?99j9a2jpggb49ha5t4mc9evpol)."
    )


def github():
    st.sidebar.markdown(
        "The entire code of the project, from source code to notebooks, is available at our GitHub repo [here](https://github.com/LucaNyckees/SCV_project1). Have a look!"
    )
...
def results_display():
    ...
    options = (
        "p-values of the Ljung Box, Box-Pierce test, and McLeod-Li",
        "ACF and PACF",
        "QQ-plot",
        "p-values of the Mann-Whitney U test",
    )

    images = ("p_values_ljung.png", "ACF_and_PACF.png", "QQ_plot.png", "mann_whitney.png")
    ...
    st.subheader(elt)
    st.image(Image.open(FIGURES_PATH / images[index]))
    descriptions(index)
...
def annual_analysis():
    ...
    st.markdown(
        r"""The Time Series of average temperatures appears to have an increasing trend over the years, but with a sudden cooling from 1962."""
    )
    st.markdown(
        r"In order to distinguish more clearly the periods that correspond to a warming or not, we present on the following figure the histogram of the annual anomalies that we have standardized."
    )

    anomalie = pd.read_csv(DATA_PATH / "generated" / "Annual" / "Annual_anomalie.csv")
    anom1 = pd.read_csv(DATA_PATH / "generated" / "Annual" / "Annual_anom1.csv")
    anom2 = pd.read_csv(DATA_PATH / "generated" / "Annual" / "Annual_anom2.csv")
    ...
    fig1 = make_subplots(
        ...
        row_heights=[1.0 / 3, 1.0 / 3, 1.0 / 3],
        column_widths=[1.0 / 10, 8.0 / 10, 1.0 / 10],
        specs=[[None, {"type": "Bar"}, None], [None, {"type": "Bar"}, None], [None, {"type": "Bar"}, None]],
        subplot_titles=[
            "Annual standardized temperature anomalies over the period 1901-2021",
    ...
    fig1.add_trace(go.Bar(x=data_Y.Years, y=anomalie.Mean, showlegend=False), row=1, col=2)
    fig1.add_trace(go.Bar(x=data_Y[data_Y.Years < 1962].Years, y=anom1.Mean, showlegend=False), row=2, col=2)
    fig1.add_trace(go.Bar(x=data_Y[data_Y.Years >= 1962].Years, y=anom2.Mean, showlegend=False), row=3, col=2)
    ...
    st.markdown(
        "In order to model our data, we will try to follow the principles of parsimony (Occam's razor) as much as possible, in order to choose the simplest model that effectively explains our data."
    )

    st.markdown(
        r"If we denote the time series of annual averages by $\{\mathbf{A}_{t}\}_{t}$ for $t = 1901,... .2021$, one of the simplest models we could propose is that our observations $\{\mathbf{A}_{t}\}_{t}$ are from a normal distribution, $\mathbf{A}_{t} \stackrel{iid}{\sim} \mathcal{N}(\mu,\sigma^{2})$. To test this we will first compare the empirical distribution of our data with the distribution of a normal distribution of mean $\mathbf{\bar{A}}$ and variance $S^{2}$."
    )

    st.markdown(
        r"On the figures below, we see that our empirical distribution is quite close to that of a normal distribution, despite the fact that we only have 121 observations."
    )
    ...
    fig = make_subplots(
        ...
        cols=3,
        row_heights=[1],
        column_widths=[1.0 / 4, 1.0 / 2, 1.0 / 4],
    ...
    with c2:
        HtmlFile = open(DATA_PATH / "html" / "tempQQplot.html", "r", encoding="utf-8")
    ...
    fig.add_trace(
        go.Scatter(x=pval_ind1.index + 1, y=pval_ind1.Ljung, mode="lines+markers", name="Ljung-Box"), row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=pval_ind1.index + 1, y=pval_ind1.McLeod, mode="lines+markers", name="McLeod-Li"), row=1, col=2
    )
    ...
    trendless_gls = pd.read_csv(DATA_PATH / "generated" / "Annual" / "Annual1_trendlessMean_GLS")

    fig.add_trace(go.Scatter(x=data_Y.Years, y=data_Y.Mean, name="Annual mean temperature"), row=1, col=2)
    fig.add_trace(go.Scatter(x=data_Y.Years, y=trendless_gls.f_without_cts, name="GLS without shift"), row=1, col=2)
    fig.add_trace(
        go.Scatter(x=data_Y.Years, y=trendless_gls.f_cts, name=r"GLS with shift", marker=dict(color="mediumvioletred")),
        row=1,
        col=2,
    )
    ...
    with col2:
        ...
        HtmlFile = open(
            DATA_PATH / "generated" / "Annual" / "Annual_summa_with_cts_personaliser", "r", encoding="utf-8"
        )
        source_code = HtmlFile.read()
        st.markdown(source_code, unsafe_allow_html=True)
    ...
    fig = make_subplots(
        ...
        cols=3,
        row_heights=[1],
        column_widths=[1.0 / 8, 3.0 / 4, 1.0 / 8],
        specs=[[None, {"type": "Scatter"}, None]],
        shared_xaxes=False,
        shared_yaxes=False,
    )
    ...
    fig["layout"].update(
        {
            ...
            "yaxis": {"title": "Trendless mean temperature (°C)"},
            "width": 1200,
            "height": 500,
        }
    )
    ...
    st.image(Image.open(FIGURES_PATH / "Detrended_mean_acf_pacf_cst.png"))

    pval_ind2 = pd.read_csv(DATA_PATH / "generated" / "Annual" / "Annual2_pvalue_indep_GLS")
    ...
    fig.add_trace(
        go.Scatter(
            x=pval_ind2.index + 1, y=pval_ind2.Ljung, mode="lines+markers", name="Ljung-Box", marker=dict(size=8)
        ),
        row=1,
        col=2,
    )
    fig.add_trace(
        go.Scatter(
            x=pval_ind2.index + 1, y=pval_ind2.McLeod, mode="lines+markers", name="McLeod-Li", marker=dict(size=8)
        ),
        row=1,
        col=2,
    )
    ...
    for i in range(4):
        fig.add_trace(
            go.Scatter(
                x=q, y=aic[aic.param_p == i].AIC, mode="lines+markers", name="p = " + str(i), marker=dict(size=15)
            ),
            row=1,
            col=2,
        )
    ...
    fig["layout"].update(
        {
            ...
            "title_x": 0.5,
            "xaxis": {"title": "Years", "zeroline": False},
            "yaxis": {"title": "Residus mean temperature(°C)"},
            "width": 1200,
            "height": 500,
        }
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=pval_ind3.index + 1, y=pval_ind3.Ljung, mode="lines+markers", name="Ljung-Box", marker=dict(size=8)
        ),
        row=1,
        col=2,
    )
    fig.add_trace(
        go.Scatter(
            x=pval_ind3.index + 1, y=pval_ind3.McLeod, mode="lines+markers", name="McLeod-Li", marker=dict(size=8)
        ),
        row=1,
        col=2,
    )
    ...
    st.markdown(
        r"Both p-values are not significant at a threshold of $\alpha = 0.05$, so we cannot reject the null hypotheses of normality and homoscedasticity. Therefore, it seems that the model we develop for the original time series of mean temperature is appropriate, in the sense that the trend that we estimate with the GLS is "
    )
