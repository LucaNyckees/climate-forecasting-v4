...
from plotly_features import (plotly_mean_temp,
                             plotly_hist_mean,
                             plotly_min,
                             plotly_max,
                             plotly_std,
...
def st_circular_vision():
    ...
    data_M = pd.DataFrame(np.array([[df[(df.Year == y) & (df.Month == m)].TG.mean(),
                                     df[(df.Year == y) & (df.Month == m)].TG.median(), df[(df.Year == y) & (df.Month == m)].TG.std(), y, int(m)] for y in Years for m in Months]),
...
def annual_analysis():
    ...
    with col2:
        ...
        HtmlFile = open(DATA_PATH / 'generated' / 'Annual' / 'Annual_summa_without_cts_personaliser', 'r', encoding='utf-8')
        source_code = HtmlFile.read()
    ...
    with col2:
        ...
        HtmlFile = open(DATA_PATH / 'generated' / 'Annual' / 'Annual_summa_with_cts_personaliser', 'r', encoding='utf-8')
        source_code = HtmlFile.read()
        st.markdown(source_code, unsafe_allow_html=True)
