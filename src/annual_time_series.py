...
data_Y = pd.DataFrame(
    np.array([[df[df.Year == y].TG.mean(), df[df.Year == y].TG.median(), df[df.Year == y].TG.std(), y] for y in Years]),
...
plt.subplots_adjust(left=0.125, bottom=0.1, right=0.9, top=1, wspace=0.4, hspace=0.6)
...
test_addfuller = adfuller(data_Y.Mean, maxlag=None, regression="nc", autolag="AIC", store=False, regresults=False)
...
# Stationarity test on annual mean with addfuller
test_addfuller = adfuller(data_Y.Mean, maxlag=None, regression="ct", autolag="AIC", store=False, regresults=False)
...
fig.suptitle("P-values of the Mann-Whitney U test", y=1.0, size="x-large", weight="roman")
...
fig = plt.figure(figsize=(8, 4), dpi=300, tight_layout=True)
fig.suptitle("P-values of the Ljung-Box ,Box-Pierce test and McLeod-Li", y=0.95, size="x-large", weight="roman")
...
def plot_test_U_MannWhitney(
    data,
    alt="two-sided",
    alpha=0.05,
    lag=1,
    loca=(0.775, 0.73),
    fname="figure/p_values_MannWhitneyTest_test.png",
    closefig=False,
):
    p_values = np.array([])
...
def f2(x):
    return coef2[0] + coef2[1] * x


def f_split(x):
    return (x < break_year) * f1(x) + (x >= break_year) * f2(x)
...
plot_acf_pacf(
    data_Y.Mean_detrended,
    lag=50,
    title="ACF and PACF of annual detrended mean temperature",
    closefig=True,
    fname="figure/Detrended_mean_acf_pacf.png",
)
...
test_addfuller = adfuller(
    data_Y.Mean_detrended, maxlag=None, regression="nc", autolag="AIC", store=False, regresults=True
)
...
plot_ljung_pierce(
    res, semilog_y=False, loca=(0.78, 0.66), df=p + q, closefig=True, fname="figure/LjungBox_Pierce_residu"
)
...
def f_cst_low(x):
    return coef_cst[0] + coef_cst[1] * (x >= break_year) + CI_trend[0] * x
...
test_addfuller = adfuller(
    data_Y.Mean_detrended, maxlag=None, regression="n", autolag="AIC", store=False, regresults=True
)
...
plot_acf_pacf(
    ...
    title="ACF and PACF of annual detrended mean temperature",
    closefig=fig,
    fname="figure/Detrended_mean_acf_pacf_cst.png",
)
...
for i, (pp, qq) in enumerate(param):
    arma_mod = ARIMA(data_Y.Mean_detrended, order=(pp, 0, qq), enforce_invertibility=False).fit(
...
fig = True
plot_ljung_pierce(
    res, semilog_y=False, loca=(0.78, 0.66), df=p + q, closefig=fig, fname="figure/LjungBox_Pierce_residu_cst"
)
norm_test = arma_mod.test_normality(method="jarquebera")
hetero_test = arma_mod.test_heteroskedasticity(method="breakvar")
...
ax.fill_between(
    np.arange(last_year + 1, last_year + delta_simulation + 1),
    CI_forecast[:, 0],
    CI_forecast[:, 1],
    alpha=0.35,
    color="green",
...
ax.plot(data_Y.Years, data_Y.Mean)
ax.fill_between(np.arange(2021, 2061), CI_forecast[:, 0], CI_forecast[:, 1], alpha=0.2, color="green")
