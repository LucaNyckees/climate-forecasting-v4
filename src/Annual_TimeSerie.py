...
plot_ljung_pierce(data_Y.Mean_detrended, semilog_y=False, loca=(0.78, 0.66), closefig=True, fname='figure/Detrended_mean_Ljung_Pierce.png')
plot_acf_pacf(data_Y.Mean_detrended, lag=50, title='ACF and PACF of annual detrended mean temperature', closefig=True, fname='figure/Detrended_mean_acf_pacf.png')
...
test_addfuller = adfuller(data_Y.Mean_detrended, maxlag=None, regression='nc', autolag='AIC', store=False, regresults=True)
...
while (test_addfuller[1] > 0.05):
    i += 1
    dens = sm.nonparametric.lowess(data_Y.Mean, np.arange(np.shape(data_Y.Mean)[0]), frac=frac[-i])
    test_addfuller = adfuller(data_Y.Mean - dens[:, 1], maxlag=None, regression='n', autolag='AIC', store=False, regresults=True)
...
test_addfuller = adfuller(data_Y.Mean_detrended, maxlag=None, regression='n', autolag='AIC', store=False, regresults=True)
...
fig = True
plot_ljung_pierce(data_Y.Mean_detrended, semilog_y=True, loca=(0.78, 0.66), closefig=fig, fname='figure/Detrended_mean_Ljung_Pierce_cst.png')
plot_acf_pacf(data_Y.Mean_detrended, lag=50, title='ACF and PACF of annual detrended mean temperature', closefig=fig, fname='figure/Detrended_mean_acf_pacf_cst.png')
